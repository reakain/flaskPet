...
from flaskpet import __version__ as flaskpet_version
from flaskpet.extensions import allows, celery, db
from flaskpet.forum.forms import UserSearchForm
from flaskpet.forum.models import Category, Forum, Post, Report, Topic
from flaskpet.management.forms import (AddForumForm, AddGroupForm, AddUserForm,
                                      CategoryForm, EditForumForm,
                                      EditGroupForm, EditUserForm)
from flaskpet.management.models import Setting, SettingsGroup
from flaskpet.plugins.models import PluginRegistry, PluginStore
from flaskpet.plugins.utils import validate_plugin
from flaskpet.user.models import Group, Guest, User
from flaskpet.utils.forms import populate_settings_dict, populate_settings_form
from flaskpet.utils.helpers import (get_online_users, register_view,
                                   render_template, time_diff, time_utcnow,
                                   FlashAndRedirect)
from flaskpet.utils.requirements import (CanBanUser, CanEditUser, IsAdmin,
                                        IsAtleastModerator,
...
class EnablePlugin(MethodView):
    ...
    def post(self, name):
        ...
        flash(
            _(
                "Plugin %(plugin)s enabled. Please restart FlaskPet now.",
                plugin=plugin.name
            ), "success"
        )
        return redirect(url_for("management.plugins"))
...
class DisablePlugin(MethodView):
    ...
    def post(self, name):
        ...
        flash(
            _(
                "Plugin %(plugin)s disabled. Please restart FlaskPet now.",
                plugin=plugin.name
            ), "success"
        )
        return redirect(url_for("management.plugins"))
