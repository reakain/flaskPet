...
# extensions
from flaskpet.extensions import (
    alembic,
    allows,
    babel,
    cache,
...
# models
from flaskpet.user.models import Guest, User

# various helpers
from flaskpet.utils.helpers import (
    app_config_from_env,
    crop_title,
    format_date,
    format_datetime,
...
from flaskpet.utils.requirements import (
    CanBanUser,
    CanEditUser,
    IsAdmin,
    IsAtleastModerator,
...
# whooshees
from flaskpet.utils.search import (
    ForumWhoosheer,
    PostWhoosheer,
    TopicWhoosheer,
    UserWhoosheer,
...
from flaskpet.utils.translations import FlaskPetDomain

from . import markup  # noqa
from .auth import views as auth_views  # noqa
#from .deprecation import FlaskPetDeprecation
...
def create_app(config=None, instance_path=None):
    ...
    app = Flask(
        "flaskpet", instance_path=instance_path, instance_relative_config=True
    )
    ...
    #app.pluggy.hook.flaskpet_additional_setup(app=app, pluggy=app.pluggy)
...
def configure_app(app, config):
    ...
    app.config.from_object("flaskpet.configs.default.DefaultConfig")
    config = get_flaskpet_config(app, config)
    # Path
    ...
    app.config.from_envvar("FLASKPET_SETTINGS", silent=True)

    # Parse the env for FLASKBB_ prefixed env variables and set
    # them on the config object
    app_config_from_env(app, prefix="FLASKPET_")
    ...
    if not app.testing:  # pragma: no branch
        warnings.simplefilter(deprecation_level, FlaskPetDeprecation)
    ...
    app.pluggy = FlaskPetPluginManager("flaskpet", implprefix="flaskpet_")
...
def configure_blueprints(app):
    app.pluggy.hook.flaskpet_load_blueprints(app=app)
...
def configure_extensions(app):
    ...
    # Flask-Themes
    themes.init_themes(app, app_identifier="flaskpet")
...
def configure_template_filters(app):
    ...
    app.pluggy.hook.flaskpet_jinja_directives(app=app)
...
def configure_context_processors(app):
    ...
    def inject_flaskpet_config():
        """Injects the ``flaskpet_config`` config variable into the
        templates.
        """
        return dict(flaskpet_config=flaskpet_config, format_date=format_date)
...
def configure_before_handlers(app):
    ...
    app.pluggy.hook.flaskpet_request_processors(app=app)
...
def configure_errorhandlers(app):
    ...
    app.pluggy.hook.flaskpet_errorhandlers(app=app)


def configure_migrations(app):
    """Configure migrations."""
    plugin_dirs = app.pluggy.hook.flaskpet_load_migrations()
...
def configure_translations(app):
    ...
    # because we of the 'flaskpet_load_translations' hook
    babel.init_app(app=app, default_domain=FlaskPetDomain(app))
    ...
    def get_locale():
        ...
        return flaskpet_config["DEFAULT_LANGUAGE"]
...
def load_plugins(app):
    ...
    # have to find all the flaskpet modules that are loaded this way
    ...
    flaskpet_modules = set(
        module
        for name, module in iteritems(sys.modules)
        if name.startswith("flaskpet")
    )
    for module in flaskpet_modules:
    ...
    try:
        ...
        app.pluggy.load_setuptools_entrypoints("flaskpet_plugins")
        return
    ...
    app.pluggy.load_setuptools_entrypoints("flaskpet_plugins")
    app.pluggy.hook.flaskpet_extensions(app=app)
    ...
    unregistered = [
        ...
        # ignore internal FlaskPet modules
        if not name.startswith("flaskpet.") and name != "flaskpet"
    ]
    with app.app_context():
