# -*- coding: utf-8 -*-
"""
    flaskpet.utils.helpers
    ~~~~~~~~~~~~~~~~~~~~~

    A few helpers that are used by flaskpet
...
from flaskpet._compat import (iteritems, range_method, text_type, string_types,
                             to_bytes, to_unicode)
from flaskpet.extensions import babel, redis_store
from flaskpet.utils.settings import flaskpet_config
...
def render_template(template, **context):  # pragma: no cover
    ...
    if current_user.is_authenticated and current_user.theme:
        ...
        theme = session.get('theme', flaskpet_config['DEFAULT_THEME'])
...
def do_topic_action(topics, user, action, reverse):  # noqa: C901
    ...
    from flaskpet.utils.requirements import (IsAtleastModeratorInForum,
                                            CanDeleteTopic, Has)
...
def get_categories_and_forums(query_result, user):
    ...
    The structure looks like this::
        [(<Category 1>,
          [(<Forum 1>, None),
           (<Forum 2>, <flaskpet.forum.models.ForumsRead at 0x38fdb50>)]),
         (<Category 2>,
          [(<Forum 3>, None),
          (<Forum 4>, None)])]
   ...
   This will print something like this:
        <Category 1>
            <Forum 1> None
            <Forum 2> <flaskpet.forum.models.ForumsRead object at 0x38fdb50>
        <Category 2>
            <Forum 3> None
            <Forum 4> None
...
def mark_online(user_id, guest=False):  # pragma: no cover
    ...
    now = int(time.time())
    expires = now + (flaskpet_config['ONLINE_LAST_MINUTES'] * 60) + 10
    if guest:
...
def get_online_users(guest=False):  # pragma: no cover
    ...
    minutes = range_method(flaskpet_config['ONLINE_LAST_MINUTES'])
    if guest:
...
def crop_title(title, length=None, suffix="..."):
    ...
    """
    length = flaskpet_config['TITLE_LENGTH'] if length is None else length
...
def time_diff():
    ...
    """
    now = time_utcnow()
    diff = now - timedelta(minutes=flaskpet_config['ONLINE_LAST_MINUTES'])
    return diff


def _get_user_locale():
    locale = flaskpet_config.get("DEFAULT_LANGUAGE", "en")
...
def check_image(url):
    ...
    If the image doesn't match the ``flaskpet_config`` settings it will
    ...
    if img_info["size"] > flaskpet_config["AVATAR_SIZE"]:
        error = "Image is too big! {}kb are allowed.".format(
            flaskpet_config["AVATAR_SIZE"]
        )
        return error, False

    if not img_info["content_type"] in flaskpet_config["AVATAR_TYPES"]:
        error = "Image type {} is not allowed. Allowed types are: {}".format(
            img_info["content_type"],
            ", ".join(flaskpet_config["AVATAR_TYPES"])
        )
        return error, False

    if img_info["width"] > flaskpet_config["AVATAR_WIDTH"]:
        error = "Image is too wide! {}px width is allowed.".format(
            flaskpet_config["AVATAR_WIDTH"]
        )
        return error, False

    if img_info["height"] > flaskpet_config["AVATAR_HEIGHT"]:
        error = "Image is too high! {}px height is allowed.".format(
            flaskpet_config["AVATAR_HEIGHT"]
        )
        return error, False
...
def get_flaskpet_config(app, config_file):
    ...
    :param config_file: A string which is either a module that can be
                        ...
                        search for a 'flaskpet.cfg' file in the instance
    ...
    if config_file is not None:
        ...
        # this walks back to flaskpet/ from flaskpet/flaskpet/cli/main.py
        project_dir = os.path.dirname(
            os.path.dirname(os.path.dirname(__file__))
        )
        project_config = os.path.join(project_dir, "flaskpet.cfg")

        # instance config doesn't exist
        instance_config = os.path.join(app.instance_path, "flaskpet.cfg")
...
class ReverseProxyPathFix(object):
    ...
    In wsgi.py::
        ...
        from flaskpet.utils.helpers import ReverseProxyPathFix
        flaskpet = create_app(config="flaskpet.cfg")
        flaskpet.wsgi_app = ReverseProxyPathFix(flaskpet.wsgi_app)
...
def enforce_recaptcha(limiter):
    ...
    if current_limit is not None:
        ...
        stats_diff = flaskpet_config["AUTH_REQUESTS"] - window_stats[1]
        login_recaptcha = stats_diff >= flaskpet_config["LOGIN_RECAPTCHA"]
...
def registration_enabled(f):
    ...
    @wraps(f)
    def wrapper(*a, **k):
        if not flaskpet_config["REGISTRATION_ENABLED"]:
...
def requires_unactivated(f):
    ...
    @wraps(f)
    def wrapper(*a, **k):
        if current_user.is_active or not flaskpet_config["ACTIVATE_ACCOUNT"]:
