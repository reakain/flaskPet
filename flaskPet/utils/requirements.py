"""
    flaskpet.utils.requirements
    ~~~~~~~~~~~~~~~~~~~~~~~~~~

    Authorization requirements for FlaskPet.

    :copyright: (c) 2015 by the FlaskPet Team.
...
from flaskpet.exceptions import FlaskPetError
from flaskpet.forum.locals import current_forum, current_post, current_topic
from flaskpet.forum.models import Forum, Post, Topic
...
class IsSameUser(IsAuthed):
    ...
    def _get_user_id_from_post(self):
        if current_post:
            ...
            raise FlaskPetError("Could not determine user")
...
class TopicNotLocked(Requirement):
    ...
    def _get_topic_from_request(self):
        if current_topic:
            ...
            raise FlaskPetError("How did you get this to happen?")
...
class ForumNotLocked(Requirement):
    ...
    def _get_forum_from_request(self):
        if current_forum:
            return current_forum
        raise FlaskPetError("Could not determine forum")
