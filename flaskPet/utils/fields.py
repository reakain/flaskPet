...
from flaskpet._compat import to_bytes, to_unicode
...
class RecaptchaValidator(object):
    ...
    def __call__(self, form, field):
        if current_app.testing or not flaskpet_config["RECAPTCHA_ENABLED"]:
            return True
    ...
    def _validate_recaptcha(self, response, remote_addr):
        ...
        try:
            private_key = flaskpet_config['RECAPTCHA_PRIVATE_KEY']
        except KeyError:
...
class RecaptchaWidget(object):
    ...
    def __call__(self, field, error=None, **kwargs):
        ...
        if not flaskpet_config["RECAPTCHA_ENABLED"]:
            return

        try:
            public_key = flaskpet_config['RECAPTCHA_PUBLIC_KEY']
        except KeyError:
