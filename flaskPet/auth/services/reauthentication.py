# -*- coding: utf-8 -*-
"""
    flaskpet.auth.services.reauthentication
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    Tools for handling reauthentication needs inside FlaskPet.
...
class DefaultFlaskPetReauthProvider(ReauthenticateProvider):
    """
    This is the default reauth provider in FlaskPet, it compares the provided
...
class PluginReauthenticationManager(ReauthenticateManager):
    """
    Default reauthentication manager for FlaskPet, it relies on plugin hooks
    ...
    def reauthenticate(self, user, secret):
        try:
            result = self.plugin_manager.hook.flaskpet_reauth_attempt(
                user=user, secret=secret
            )
            if not result:
                raise StopAuthentication(_("Wrong password."))
            self.plugin_manager.hook.flaskpet_post_reauth(user=user)
        except StopAuthentication:
            self.plugin_manager.hook.flaskpet_reauth_failed(user=user)
            raise
        finally:
            try:
                self.session.commit()
