# -*- coding: utf-8 -*-
"""
    flaskpet.auth.services.authentication
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
class DefaultFlaskPetAuthProvider(AuthenticationProvider):
    """
...
class PluginAuthenticationManager(AuthenticationManager):
    ...
    process. This is the default authentication manager for FlaskPet.
    """
    ...
    def authenticate(self, identifier, secret):
        try:
            user = self.plugin_manager.hook.flaskpet_authenticate(
                identifier=identifier, secret=secret
            )
            if user is None:
                raise StopAuthentication(_("Wrong username or password."))
            self.plugin_manager.hook.flaskpet_post_authenticate(user=user)
            return user
        except StopAuthentication:
            self.plugin_manager.hook.flaskpet_authentication_failed(
