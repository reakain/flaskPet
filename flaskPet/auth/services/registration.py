...
"""
    ...
    Implementation of services found in flaskpet.core.auth.services
...
class SendActivationPostProcessor(RegistrationPostProcessor):
    ...
    :type account_activator: :class:`~flaskpet.core.auth.activation.AccountActivator`
    """  # noqa
...
class AutoActivateUserPostProcessor(RegistrationPostProcessor):
    ...
    :param config: Current flaskpet configuration object
    """
...
class RegistrationService(UserRegistrationService):
    """
    Default registration service for FlaskPet, runs the registration information
    against the provided validators and if it passes, creates the user.

    If any of the provided
    :class:`UserValidators<flaskpet.core.auth.registration.UserValidator>`
    raise a :class:`ValidationError<flaskpet.core.exceptions.ValidationError>`
    then the register method will raise a
    :class:`StopValidation<flaskpet.core.exceptions.StopValidation>` with all
    ...
    def _validate_registration(self, user_info):
        failures = []
        validators = self.plugins.hook.flaskpet_gather_registration_validators()
    ...
    def _handle_failure(self, user_info, failures):
        self.plugins.hook.flaskpet_registration_failure_handler(
    ...
    def _post_process(self, user):
        self.plugins.hook.flaskpet_registration_post_processor(user=user)
