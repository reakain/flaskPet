...
"""
    ...
    resetting the password of a user if they has lost their password
...
from flaskpet.auth.forms import (
...
from flaskpet.extensions import db, limiter
...
class Register(MethodView):
    ...
    def form(self):
        current_app.pluggy.hook.flaskpet_form_registration(form=RegisterForm)
        form = RegisterForm()

        form.language.choices = get_available_languages()
        form.language.default = flaskpet_config['DEFAULT_LANGUAGE']
...
def flaskpet_load_blueprints(app):
    ...
    def login_rate_limit():
        ...
        return "{count}/{timeout}minutes".format(
            count=flaskpet_config["AUTH_REQUESTS"],
            timeout=flaskpet_config["AUTH_TIMEOUT"]
        )
    ...
    def check_rate_limiting():
        ...
        if not flaskpet_config["AUTH_RATELIMIT_ENABLED"]:
            return None
