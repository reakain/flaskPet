...
from flaskpet import __version__
from flaskpet.utils.populate import create_user, update_user


_email_regex = r"[^@]+@[^@]+\.[^@]+"


class FlaskPetCLIError(click.ClickException):
...
def validate_plugin(plugin):
    ...
    if plugin not in current_app.pluggy.list_name():
        raise FlaskPetCLIError("Plugin {} not found.".format(plugin), fg="red")
    return True
...
def validate_theme(theme):
    ...
    try:
        ...
        raise FlaskPetCLIError("Theme {} not found.".format(theme), fg="red")
...
def get_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    message = ("FlaskPet %(version)s using Flask %(flask_version)s on "
