...
from flaskpet import create_app
from flaskpet.cli.utils import (EmailType, FlaskPetCLIError, get_version,
                               prompt_config_path, prompt_save_user,
                               write_config)
from flaskpet.extensions import alembic, celery, db, whooshee
from flaskpet.utils.populate import (create_default_groups,
                                    create_default_settings, create_latest_db,
                                    create_test_data, create_welcome_forum,
                                    insert_bulk_data, run_plugin_migrations,
                                    update_settings_from_fixture)
from flaskpet.utils.translations import compile_translations
...
class FlaskPetGroup(FlaskGroup):
    def __init__(self, *args, **kwargs):
        super(FlaskPetGroup, self).__init__(*args, **kwargs)
        self._loaded_flaskpet_plugins = False

    def _load_flaskpet_plugins(self, ctx):
        if self._loaded_flaskpet_plugins:
            return

        try:
            app = ctx.ensure_object(ScriptInfo).load_app()
            app.pluggy.hook.flaskpet_cli(cli=self, app=app)
            self._loaded_flaskpet_plugins = True
        except Exception:
            logger.error(
            ...
            shell_context_processors = app.pluggy.hook.flaskpet_shell_context()
            for p in shell_context_processors:
                app.shell_context_processor(p)

    def get_command(self, ctx, name):
        self._load_flaskpet_plugins(ctx)
        return super(FlaskPetGroup, self).get_command(ctx, name)

    def list_commands(self, ctx):
        self._load_flaskpet_plugins(ctx)
        return super(FlaskPetGroup, self).list_commands(ctx)
...
@click.group(cls=FlaskPetGroup, create_app=make_app, add_version_option=False,
...
@click.option("--config", expose_value=False, callback=set_config,
              ...
              help="Specify the config to use either in dotted module "
                   "notation e.g. 'flaskpet.configs.default.DefaultConfig' "
                   "or by using a path like '/path/to/flaskpet.cfg'")
...
@click.option("--version", expose_value=False, callback=get_version,
              is_flag=True, is_eager=True, help="Show the FlaskPet version.")
@click.pass_context
@click_log.simple_verbosity_option(logger)
def flaskpet(ctx):
    """This is the commandline interface for flaskpet."""
...
flaskpet.add_command(alembic_click, "db")
...
def install(welcome, force, username, email, password, no_plugins):
    """Installs flaskpet. If no arguments are used, an interactive setup
    will be run.
    """
    click.secho("[+] Installing FlaskPet...", fg="cyan")
    ...
    click.secho("[+] FlaskPet has been successfully installed!",
                fg="green", bold=True)
...
def populate(bulk_data, test_data, posts, topics, force, initdb):
    """Creates the necessary tables and groups for FlaskPet."""
    if force:
    ...
    if fixture or all_latest:
        try:
            settings = import_string(
                "flaskpet.fixtures.{}".format(fixture)
            )
            settings = settings.fixture
        except ImportError:
            raise FlaskPetCLIError("{} fixture is not available"
...
@flaskpet.command("celery", add_help_option=False,
...
def start_celery(ctx):
    ...
    CeleryCommand(celery).execute_from_commandline(
        ["flaskpet celery"] + ctx.args
    )


@flaskpet.command("shell", short_help="Runs a shell in the app context.")
@with_appcontext
def shell_command():
...
@flaskpet.command("urls", short_help="Show routes for the app.")
...
def generate_config(development, output, force):
    ...
    if os.path.exists(config_path) and not os.path.isfile(config_path):
        config_path = os.path.join(config_path, "flaskpet.cfg")

    # An override to handle database location paths on Windows environments
    database_path = "sqlite:///" + os.path.join(
        os.path.dirname(current_app.instance_path), "flaskpet.sqlite")
    if os.name == 'nt':
    ...
    default_conf = {
        ...
        "mail_username": "",
        "mail_password": "",
        "mail_sender_name": "FlaskPet Mailer",
    ...
    click.secho("The name and port number of the exposed server.\n"
                "If FlaskPet is accesible on port 80 you can just omit the "
                "port.\n For example, if FlaskPet is accessible via "
    ...
    # HTTPS or HTTP
    click.secho("Is HTTPS (recommended) or HTTP used for to serve FlaskPet?",
                fg="cyan")
    ...
    click.secho("For Postgres use:\n"
                "    postgresql://flaskpet@localhost:5432/flaskpet\n"
