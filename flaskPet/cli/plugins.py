...
from flaskpet.extensions import db
from flaskpet.cli.main import flaskpet
from flaskpet.cli.utils import validate_plugin, get_cookiecutter
from flaskpet.plugins.models import PluginRegistry, PluginStore
from flaskpet.plugins.utils import remove_zombie_plugins_from_db
...
@with_appcontext
def cleanup():
    """Removes zombie plugins from FlaskPet.
    ...
    if len(deleted_plugins) > 0:
        click.secho("[+] Removed following zombie plugins from FlaskPet: ",
                    fg="green", nl=False)
...
def new_plugin(template, out_dir, force):
    ...
    https://github.com/sh4nks/cookiecutter-flaskpet-plugin.
