...
@spec
def flaskpet_load_blueprints(app):
...
@spec
def flaskpet_request_processors(app):
...
@spec
def flaskpet_jinja_directives(app):
...
@spec
def flaskpet_additional_setup(app, pluggy):
    ...
    For example, you could apply a WSGI middleware::
        ...
        @impl
        def flaskpet_additional_setup(app):
...
@spec
def flaskpet_load_post_markdown_class(app):
    """
    ...
    Example::
        ...
        @impl
        def flaskpet_load_post_markdown_class():
...
@spec
def flaskpet_load_nonpost_markdown_class(app):
    """
    Hook for loading a mistune renderer child class in order to render
    ...
    Example::
        ...
        @impl
        def flaskpet_load_nonpost_markdown_class():
            return YellingRenderer
...
@spec
def flaskpet_cli(cli, app):
    """Hook for registering CLI commands.

    For example::

        @impl
        def flaskpet_cli(cli):
            @cli.command()
            def testplugin():
    ...
    :param app: The application object.
    :param cli: The FlaskPetGroup CLI object.
    """


@spec
def flaskpet_shell_context():
...
# Event hooks
@spec
def flaskpet_event_post_save_before(post):
    """Hook for handling a post before it has been saved.

    :param flaskpet.forum.models.Post post: The post which triggered the event.
    """


@spec
def flaskpet_event_post_save_after(post, is_new):
...
@spec
def flaskpet_event_topic_save_before(topic):
    """Hook for handling a topic before it has been saved.

    :param flaskpet.forum.models.Topic topic: The topic which triggered the
                                             event.
    """


@spec
def flaskpet_event_topic_save_after(topic, is_new):
...
@spec
def flaskpet_event_user_registered(username):
    """Hook for handling events after a user is registered

    .. warning::

        This hook is deprecated in favor of
        :func:`~flaskpet.plugins.spec.flaskpet_registration_post_processor`
...
def flaskpet_gather_registration_validators():
    ...
    :class:`~flaskpet.core.auth.registration.UserRegistrationInfo` and raises
    a :class:`~flaskpet.core.exceptions.ValidationError` if the registration
    is invalid or :class:`~flaskpet.core.exceptions.StopValidation` if
    ...
    See Also: :class:`~flaskpet.core.auth.registration.UserValidator`
    """


@spec
def flaskpet_registration_failure_handler(user_info, failures):
    """
    ...
    See Also: :class:`~flaskpet.core.auth.registration.RegistrationFailureHandler`
    """  # noqa
...
def flaskpet_authenticate(identifier, secret):
    ...
    Example of ending authentication::
        ...
        @impl(tryfirst=True)
        def flaskpet_authenticate(user, identifier):
...
def flaskpet_post_authenticate(user):
    ...
    See also:
    :class:`PostAuthenticationHandler<flaskpet.core.auth.PostAuthenticationHandler>`
...
def flaskpet_authentication_failed(identifier):
    ...
    See also:
    :class:`AuthenticationFailureHandler<flaskpet.core.auth.AuthenticationFailureHandler>`
...
def flaskpet_reauth_attempt(user, secret):
    """Hook for handling reauth in FlaskPet
    ...
    See also:
    :class:`ReauthenticateProvider<flaskpet.core.auth.ReauthenticateProvider>`
...
def flaskpet_post_reauth(user):
    ...
    These hooks are called a user has passed the flaskpet_reauth_attempt
    ...
    See also:
    :class:`PostReauthenticateHandler<flaskpet.core.auth.PostAuthenticationHandler>`
    """
...
def flaskpet_reauth_failed(user):
    ...
    for flaskpet_reauth_attempt returns a True result or if
    ...
    See also:
    :class:`ReauthenticateFailureHandler<flaskpet.core.auth.ReauthenticateFailureHandler>`
    """


# Form hooks
@spec
def flaskpet_form_new_post(form):
    """Hook for modifying the :class:`~flaskpet.forum.forms.ReplyForm`.
    ...
    :param form: The :class:`~flaskpet.forum.forms.ReplyForm` class.
    """


@spec
def flaskpet_form_new_post_save(form):
    """Hook for modifying the :class:`~flaskpet.forum.forms.ReplyForm`.
...
@spec
def flaskpet_form_new_topic(form):
    """Hook for modifying the :class:`~flaskpet.forum.forms.NewTopicForm`

    :param form: The :class:`~flaskpet.forum.forms.NewTopicForm` class.
    """


@spec
def flaskpet_form_new_topic_save(form, topic):
    """Hook for modifying the :class:`~flaskpet.forum.forms.NewTopicForm`.
...
@spec
def flaskpet_form_registration(form):
    """
    Hook for modifying the :class:`~flaskpet.auth.forms.RegisterForm`.
...
@spec
def flaskpet_gather_password_validators(app):
    """
    Hook for gathering :class:`~flaskpet.core.changesets.ChangeSetValidator`
    instances specialized for handling :class:`~flaskpet.core.user.update.PasswordUpdate`
...
@spec
def flaskpet_gather_email_validators(app):
    """
    Hook for gathering :class:`~flaskpet.core.changesets.ChangeSetValidator`
    instances specialized for :class:`~flaskpet.core.user.update.EmailUpdate`.
    This hook should return an iterable::
...
@spec
def flaskpet_gather_details_update_validators(app):
    """
    Hook for gathering :class:`~flaskpet.core.changesets.ChangeSetValidator`
    instances specialized for :class:`~flaskpet.core.user.update.UserDetailsChange`.
    This hook should return an iterable::
        ...
        @impl
        def flaskpet_gather_details_update_validators(app):
            renderer = app.pluggy.hook.flaskpet_load_nonpost_markdown_class()
...
@spec
def flaskpet_password_updated(user):
    """
    ...
    after the password change has been persisted::
        ...
        @impl
        def flaskpet_password_updated(app, user):
            send_email(
                "Password changed",
                [user.email],
                text_body=...,
...
@spec
def flaskpet_email_updated(user, email_update):
    """
    ...
    the email change has been persisted::
        ...
        @impl
        def flaskpet_email_updated(app):
            send_email(
                "Email changed",
...
@spec
def flaskpet_settings_updated(user, settings_update):
    """
...
# Template Hooks
@spec
def flaskpet_tpl_navigation_before():
...
@spec
def flaskpet_tpl_navigation_after():
    """Hook for registering additional navigation items.
...
@spec
def flaskpet_tpl_user_nav_loggedin_before():
...
@spec
def flaskpet_tpl_user_nav_loggedin_after():
    """Hook for registering additional user navigational items
    which are only shown when a user is logged in.
...
@spec
def flaskpet_tpl_form_registration_before(form):
...
@spec
def flaskpet_tpl_form_registration_after(form):
...
@spec
def flaskpet_tpl_form_user_details_before(form):
...
@spec
def flaskpet_tpl_form_user_details_after(form):
...
@spec
def flaskpet_tpl_profile_settings_menu(user):
    ...
    For example::
        ...
        @impl(trylast=True)
        def flaskpet_tpl_profile_settings_menu():
            return [
    ...
    Hookwrappers for this spec should not be registered as FlaskPet
...
@spec
def flaskpet_tpl_profile_sidebar_links(user):
    """
    ...
    :class:`~flaskpet.display.navigation.NavigationItem` instances::
    ...
    .. warning::
        Hookwrappers for this spec should not be registered as FlaskPet registers
...
def flaskpet_tpl_admin_settings_menu(user):
    ...
    For example::
        ...
        @impl(trylast=True)
        def flaskpet_tpl_admin_settings_menu():
...
@spec
def flaskpet_tpl_profile_sidebar_stats(user):
...
@spec
def flaskpet_tpl_post_author_info_before(user, post):
...
@spec
def flaskpet_tpl_post_author_info_after(user, post):
...
@spec
def flaskpet_tpl_post_content_before(post):
...
@spec
def flaskpet_tpl_post_content_after(post):
...
@spec
def flaskpet_tpl_post_menu_before(post):
...
@spec
def flaskpet_tpl_post_menu_after(post):
...
@spec
def flaskpet_tpl_topic_controls(topic):
...
@spec
def flaskpet_tpl_form_new_post_before(form):
    ...
    For example::
        ...
        @impl
        def flaskpet_tpl_form_new_post_after(form):
...
@spec
def flaskpet_tpl_form_new_topic_before(form):
    """Hook for inserting a new form field before the first field is
...
@spec
def flaskpet_tpl_form_new_topic_after(form):
    """Hook for inserting a new form field after the last field is
