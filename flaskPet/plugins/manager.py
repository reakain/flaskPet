# -*- coding: utf-8 -*-
"""
    flaskpet.plugins.manager
    ~~~~~~~~~~~~~~~~~~~~~~~

    Plugin Manager for FlaskPet

    :copyright: 2017, the FlaskPet Team
...
from flaskpet.utils.helpers import parse_pkg_metadata


logger = logging.getLogger(__name__)


class FlaskPetPluginManager(pluggy.PluginManager):
    """Overwrites :class:`pluggy.PluginManager` to add FlaskPet
    specific stuff.
    """

    def __init__(self, project_name, implprefix=None):
        super(FlaskPetPluginManager, self).__init__(
            project_name=project_name, implprefix=implprefix
        )
        self._plugin_metadata = {}
        self._disabled_plugins = {}

        # we maintain a seperate dict for flaskpet.* internal plugins
    ...
    def register(self, plugin, name=None, internal=False):
        ...
        name = super(FlaskPetPluginManager, self).register(plugin, name)
        if not internal:
            return name
    ...
    def unregister(self, plugin=None, name=None):
        ...
        """
        plugin = super(FlaskPetPluginManager, self).unregister(
    ...
    def set_blocked(self, name):
        ...
        registered.
        """
        super(FlaskPetPluginManager, self).set_blocked(name)
        self._internal_name2plugin[name] = None

    def is_blocked(self, name):
        """Return True if the name blockss registering plugins of that name."""
        blocked = super(FlaskPetPluginManager, self).is_blocked(name)
    ...
    def get_plugin(self, name):
        ...
        plugin = super(FlaskPetPluginManager, self).get_plugin(name)
        return self._internal_name2plugin.get(name, plugin)

    def get_name(self, plugin):
        """Return name for registered plugin or None if not registered."""
        name = super(FlaskPetPluginManager, self).get_name(plugin)
        if name:
            return name
