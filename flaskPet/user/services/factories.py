# -*- coding: utf-8 -*-
"""
    flaskpet.user.services.factories
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    Factory functions for the various FlaskPet user services.

    These factories are provisional and considered private APIs.

    :copyright: 2018, the FlaskPet Team.
...
def details_update_factory():
    validators = list(
        chain.from_iterable(
            current_app.pluggy.hook.flaskpet_gather_details_update_validators(
                app=current_app
            )
        )
    )
...
def password_update_handler():
    validators = list(
        chain.from_iterable(
            current_app.pluggy.hook.flaskpet_gather_password_validators(app=current_app)
        )
    )
...
def email_update_handler():
    validators = list(
        chain.from_iterable(
            current_app.pluggy.hook.flaskpet_gather_email_validators(app=current_app)
        )
    )
