...
class ChangeSetValidator(ABC):
    ...
    def validate(self, model, changeset):
        """
        May raise a :class:`~flaskpet.core.exceptions.ValidationError`
        to signify that the changeset cannot be applied to the model.
        Or a :class:`~flaskpet.core.exceptions.StopValidation` to immediately
        halt all validation.
        """
        pass
...
class ChangeSetHandler(ABC):
    ...
    def apply_changeset(self, model, changeset):
        ...
        :class:`~flaskpet.core.exceptions.StopValidation` if the changeset
