# -*- coding: utf-8 -*-
"""
    flaskpet.core.auth.authentication
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
from ..exceptions import BaseFlaskPetError


class StopAuthentication(BaseFlaskPetError):
    """
...
class ForceLogout(BaseFlaskPetError):
    """
...
class AuthenticationManager(ABC):
    ...
    def authenticate(self, identifier, secret):
        ...
        :rtype: :class:`User<flaskpet.user.models.User>`
...
class AuthenticationProvider(ABC):
    """
    Used to provide an authentication service for FlaskPet.
    ...
    source::
        ...
        class LDAPAuthenticationProvider(AuthenticationProvider):
            ...
            def authenticate(self, identifier, secret):
                user_dn = "uid={},ou=flaskpet,ou=org".format(identifier)
                try:
    ...
    :class:`StopAuthentication<flaskpet.core.auth.authentication.StopAuthentication>`
...
class PostAuthenticationHandler(ABC):
    ...
    :class:`~flaskpet.core.auth.authentication.AuthenticationFailureHandler`
    instances to be run.
    ...
    def handle_post_auth(self, user):
        ...
        :type user: :class:`User<flaskpet.user.model.User>`
        """
        pass
...
class ReauthenticateManager(ABC):
    """
    Used to handle the reauthentication process in FlaskPet. A default
    ...
    def reauthenticate(self, user, secret):
        ...
        :type user: :class:`User<flaskpet.user.models.User>`
        """
        pass
...
class ReauthenticateProvider(ABC):
    ...
    :class:`~flaskpet.core.auth.authentication.AuthenticationProvider`
    ...
    :class:~flaskpet.core.auth.authentication.StopAuthentication`
    ...
    An example::
        ...
        class LDAPReauthenticateProvider(ReauthenticateProvider):
            ...
            def reauthenticate(self, user, secret):
                user_dn = "uid={},ou=flaskpet,ou=org".format(user.username)
                try:
                    self.ldap_client.bind_user(user_dn, secret)
                    return True
                except Exception:
...
class ReauthenticateFailureHandler(ABC):
    """
    Used to manager reauthentication failures in FlaskPet.
...
class PostReauthenticateHandler(ABC):
    ...
    :class:`~flaskpet.core.auth.authentication.PostAuthenticationHandler`,
