...
from flaskpet.utils.database import (CRUDMixin, HideableCRUDMixin, UTCDateTime,
                                    make_comparable)
from flaskpet.utils.helpers import (get_categories_and_forums, get_forums,
...
class Post(HideableCRUDMixin, db.Model):
    ...
    def save(self, user=None, topic=None):
        ...
        """
        current_app.pluggy.hook.flaskpet_event_post_save_before(post=self)

        # update/edit the post
        if self.id:
            db.session.add(self)
            db.session.commit()
            current_app.pluggy.hook.flaskpet_event_post_save_after(post=self,
                                                                  is_new=False)
            return self
...
class Topic(HideableCRUDMixin, db.Model):
    ...
    def save(self, user=None, forum=None, post=None):
        ...
        current_app.pluggy.hook.flaskpet_event_topic_save_after(topic=self,
                                                               is_new=True)
        return self
