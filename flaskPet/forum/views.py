...
from flaskpet.extensions import allows, db
from flaskpet.markup import make_renderer
from flaskpet.forum.forms import (NewTopicForm, QuickreplyForm, ReplyForm,
                                 ReportForm, SearchPageForm, UserSearchForm)
from flaskpet.forum.models import (Category, Forum, ForumsRead, Post, Topic,
                                  TopicsRead)
from flaskpet.user.models import User
from flaskpet.utils.helpers import (do_topic_action, format_quote,
                                   get_online_users, real, register_view,
                                   render_template, time_diff, time_utcnow,
                                   FlashAndRedirect)
from flaskpet.utils.requirements import (CanAccessForum,
...
class ViewTopic(MethodView):
    ...
    def get(self, topic_id, slug=None):
        ...
        posts = Post.query.outerjoin(
            ...
            Post.id.asc()
        ).paginate(page, flaskpet_config["POSTS_PER_PAGE"], False)
    ...
    def post(self, mode=None):
        ...
        if mode == "nonpost":
            render_classes = current_app.pluggy.hook.\
                flaskpet_load_nonpost_markdown_class(
                    app=current_app
                )
        else:
            render_classes = current_app.pluggy.hook.\
                flaskpet_load_post_markdown_class(
                    app=current_app
                )
