...
class FlaskPetTokenSerializer(tokens.TokenSerializer):
    """
    Default token serializer for FlaskPet. Generates JWTs
    that are time sensitive. By default they will expire after
    1 hour.

    It creates tokens from flaskpet.core.tokens.Token instances
    and creates instances of that class when loading tokens.

    When loading a token, if an error occurs related to the
    token itself, a flaskpet.core.tokens.TokenError will be
    ...
    def dumps(self, token):
        """
        Transforms an instance of flaskpet.core.tokens.Token into
        a text serialized JWT.

        :flaskpet.core.tokens.Token token: Token to transformed into a JWT
    ...
    def loads(self, raw_token):
        """
        Transforms a JWT into a flaskpet.core.tokens.Token.

        If a token is invalid due to it being malformed,
        tampered with or expired, a flaskpet.core.tokens.TokenError
        is raised. Errors not related to token parsing are
        simply propagated.

        :str raw_token: JWT to be parsed
        :returns flaskpet.core.tokens.Token: Parsed token
        """
        try:
